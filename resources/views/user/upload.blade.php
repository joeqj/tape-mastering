...
@section('content')
    <section class="py-section-sm">
        ...
        <form action="{{ url('/store') }}" method="POST" enctype="multipart/form-data" class="flex gap-24 py-section-sm"
            x-data="orderForm">
            @csrf
            ...
            <aside class="lg:w-1/3 space-y-8">
                ...
                <div class="max-w-sm">
                    <label for="coupon"
                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Coupon</label>
                    <div class="relative">
                        <input type="text" id="coupon" name="coupon" x-ref="coupon"
                            class="block w-full p-4 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" />
                        <button @click.prevent="checkCoupon()" class="btn-inline">Apply</button>
                    </div>
                    <p x-show="hasError" x-text="error" class="text-red-500 mt-2" x-cloak></p>
                </div>
                <p class="text-sm">Currency conversion will take place in the next step</p>
            </aside>
        </form>
